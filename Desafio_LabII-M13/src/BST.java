...
public class BST {
	...
	protected int cNodes = 0, cLeaf = 0;
	...
	/**
	 * Caminhamento em pós-ordem. Visita a sub-arvore esquerda Visita a sub-arvore
	 * direita Visista o nodo
	 **/
	...
	private void countNodes(BSTNode p) {
		if (p != null) {
			cNodes++;
			countNodes(p.left);
			countNodes(p.right);
	...
	private void countLeafNodes(BSTNode p) {
		if (p != null) {
	...
	public boolean isFull() {
		if (countNodes() == (int) Math.pow(2, height(root)) - 1)
			return true;
		else
			return false;
	}
